...
from app.services.ai_services import generate_graph
from app.services.ai_services.generate_graph import (
    MissingAPIKeyError,
    PipelineConfig,
    merge_graphs,
    process_markdown,
...
def test_process_markdown_empty_file_returns_empty_graph(tmp_path):
    ...
    with patch(
        "app.services.ai_services.generate_graph._get_client"
    ) as mock_get_client:
...
def test_process_markdown_extracts_merges_and_refines(tmp_path):
    ...
    with (
        patch("app.services.ai_services.generate_graph._get_client", return_value=MagicMock()),
        patch(
            "app.services.ai_services.generate_graph._create_extract_with_retry",
            return_value=fake_extract,
        ),
        patch(
            "app.services.ai_services.generate_graph.split_text_content",
            return_value=["chunk-1", "chunk-2"],
        ),
        patch(
            "app.services.ai_services.generate_graph.refine_graph_with_llm",
