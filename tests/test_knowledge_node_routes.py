...
- POST /graphs/{graph_id}/questions - Create question for a node
"""
...
class TestCreateKnowledgeNode:
    """Test cases for POST /graphs/{graph_id}/nodes endpoint."""
    ...
    async def test_create_node_success_minimal(
        ...
        response = await authenticated_client.post(
            f"/graphs/{private_graph_in_db.id}/nodes",
            json=node_data,
        )
    ...
    async def test_create_node_invalid_graph_id_format(
        ...
        response = await authenticated_client.post(
            "/graphs/invalid-uuid-format/nodes",
            json=node_data,
        )
...
class TestCreatePrerequisite:
    """Test cases for POST /graphs/{graph_id}/prerequisites endpoint."""
    ...
    async def test_create_prerequisite_success_default_weight(
        ...
        response = await authenticated_client.post(
            f"/graphs/{graph_id}/prerequisites",
            json=prereq_data,
        )
    ...
    async def test_create_prerequisite_invalid_graph_id(
        ...
        response = await authenticated_client.post(
            "/graphs/invalid-uuid/prerequisites",
            json=prereq_data,
        )
    ...
    async def test_create_prerequisite_graph_not_found(
        ...
        response = await authenticated_client.post(
            f"/graphs/{fake_graph_id}/prerequisites",
            json=prereq_data,
        )
...
class TestCreateSubtopic:
    """Test cases for POST /graphs/{graph_id}/subtopics endpoint."""

    @pytest.mark.asyncio
    async def test_create_subtopic_success_default_weight(
        self,
        authenticated_client: AsyncClient,
        private_graph_with_few_nodes_and_relations_in_db: dict,
    ):
        """Test creating a subtopic with default weight."""
        graph_data = private_graph_with_few_nodes_and_relations_in_db
        graph_id = graph_data["graph"].id
        nodes = graph_data["nodes"]

        subtopic_data = {
            "parent_node_id": str(nodes["integrals"].id),
            "child_node_id": str(nodes["chain-rule"].id),
        }

        response = await authenticated_client.post(
            f"/graphs/{graph_id}/subtopics",
            json=subtopic_data,
        )

        assert response.status_code == status.HTTP_201_CREATED
        data = response.json()
        assert data["parent_node_id"] == subtopic_data["parent_node_id"]
        assert data["child_node_id"] == subtopic_data["child_node_id"]
        assert data["graph_id"] == str(graph_id)
        assert data["weight"] == 1.0  # Default weight

    @pytest.mark.asyncio
    async def test_create_subtopic_success_custom_weight(
        self,
        authenticated_client: AsyncClient,
        private_graph_with_few_nodes_and_relations_in_db: dict,
    ):
        """Test creating a subtopic with custom weight."""
        graph_id = private_graph_with_few_nodes_and_relations_in_db["graph"].id
        nodes = private_graph_with_few_nodes_and_relations_in_db["nodes"]
    ...
    @pytest.mark.asyncio
    async def test_create_subtopic_duplicate(
        self,
        authenticated_client: AsyncClient,
        private_graph_with_few_nodes_and_relations_in_db: dict,
    ):
        """Test that duplicate subtopic returns 409 conflict."""
        graph_id = private_graph_with_few_nodes_and_relations_in_db["graph"].id
        nodes = private_graph_with_few_nodes_and_relations_in_db["nodes"]

        # This subtopic already exists in the fixture
        subtopic_data = {
            "parent_node_id": str(nodes["calculus-basics"].id),
            "child_node_id": str(nodes["derivatives"].id),
        }
...
class TestCreateQuestion:
    """Test cases for POST /graphs/{graph_id}/questions endpoint."""
