...
async def test_update_mastery_from_grading_question_not_found(
    ...
    user = User(id=uuid4())
    question_id = uuid4()
    GRADING_RESULT_DUMMY = GradingResult(
        question_id=str(question_id),
        is_correct=True,
    ...
    result = await mastery_service.update_mastery_from_grading(
        db_session, user, question_id, GRADING_RESULT_DUMMY
    )
...
async def test_update_mastery_from_grading_node_not_found(
    ...
    user = User(id=uuid4())
    question_id = uuid4()
    GRADING_RESULT_DUMMY_2 = GradingResult(
        question_id=str(question_id),
        is_correct=True,
        correct_answer="Paris",
        p_g=0.0,
    ...
    result = await mastery_service.update_mastery_from_grading(
        db_session, user, question_id, GRADING_RESULT_DUMMY_2
    )
