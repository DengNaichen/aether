...
class TestEnrollmentAPI:
    ...
    async def test_enroll_a_course_failed_with_already_enrolled(
        ...
        """
        response = await enrolled_user_client.post(f"/courses/{course_in_db.id}"
                                                   f"/enrollments")
    ...
    async def test_enroll_a_course_successful_as_authenticated_user(
        ...
        course_id = COURSE_ID
        response = await authenticated_client.post(f"/courses/{course_id}/"
                                                   f"enrollments")

        assert response.status_code == status.HTTP_201_CREATED

        enrollment_query = select(Enrollment).where(
            Enrollment.course_id == course_id, Enrollment.user_id == user_in_db.id
        )
