...
def get_multiple_choice_details():
    ...
    while True:
        num_options_str = input(f"please select options number (2-{len(OPTIONS_KEYS)}): ").strip()
    ...
    while True:
        answer_key = get_required_input(f"请输入正确答案的字母 ({', '.join(valid_keys)}): ").strip().upper()
        if answer_key in valid_keys:
            details["answer"] = {"key": answer_key}
            break
        else:
            print(f"error: answer must be one of you options {valid_keys}, please try again.")
    return details
        ...
        with open(JSON_FILE_PATH, 'r', encoding='utf-8') as f:
            try:
                ...
                if content: # 确保文件不是空的
                    ...
                    # 确保基本结构存在
                    if "problems" not in all_data: all_data["problems"] = []
                    if "relationships" not in all_data: all_data["relationships"] = []
