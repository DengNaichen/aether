...
KNOWLEDGE_FILES = ["./g11_chem/chapter2.json"]
...
def create_graph_in_neo4j(driver, data):
    with driver.session(database=DATABASE) as session:
        ...
        for rel in data["relationships"]:
            source_exists = any(
                node["id"] == rel["source_id"] for node in data["nodes"]
            )
            target_exists = any(
                node["id"] == rel["target_id"] for node in data["nodes"]
            )
            ...
            if source_exists and target_exists:
                ...
                print(
                    f"  - 警告: 跳过关系 {rel['source_id']}->{rel['target_id']}，因为节点未在数据中定义。"
                )
