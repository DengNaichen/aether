...
## Status Snapshot
- Backend: FastAPI + PostgreSQL + FSRS mastery (no BKT), Supabase JWT auth, graph CRUD, questions (MC/fill/calculation), adaptive recommendation, PDF/MD → graph/question pipeline (Gemini 2.5 Flash + Gemini 3 Pro).
...
- Graph RAG (增量 + QA): incremental ingest of new docs into existing graphs (dedupe/merge), vector index over nodes/chunks, and graph-aware QA endpoint that cites nodes/edges.
