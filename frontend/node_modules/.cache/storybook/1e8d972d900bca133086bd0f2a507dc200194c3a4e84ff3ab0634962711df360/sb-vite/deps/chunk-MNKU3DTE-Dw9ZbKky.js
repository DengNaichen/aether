import { __toDynamicImportESM, __toESM as __toESM$1 } from "./chunk-B4p5S3KX.js";
...
//#region node_modules/@storybook/react/dist/chunk-MNKU3DTE.mjs
...
var require_semver = __commonJS({ "../../node_modules/semver/classes/semver.js"(exports, module) {
	var debug = require_debug(), { MAX_LENGTH, MAX_SAFE_INTEGER } = require_constants(), { safeRe: re, t } = require_re(), parseOptions = require_parse_options(), { compareIdentifiers } = require_identifiers();
...
var getAct = async ({ disableAct = !1 } = {}) => {
	...
	if (typeof clonedReact.act == "function") reactAct = clonedReact.act;
		...
		let deprecatedTestUtils = await import("./react-dom_test-utils.js").then(__toDynamicImportESM(1));
...
var render = (args, context) => {
	let { id, component: Component$1 } = context;
	if (!Component$1) throw new Error(`Unable to render story ${id} as the component annotation is missing from the default export`);
	return import_react.createElement(Component$1, { ...args });
};
...
async function renderToCanvas({ storyContext, unboundStoryFn, showMain, showException, forceRemount }, canvasElement) {
	let { renderElement, unmountElement } = await import("./react-18-CbmTboQB.js"), Story = unboundStoryFn, content = storyContext.parameters.__isPortableStory ? import_react.createElement(Story, { ...storyContext }) : import_react.createElement(ErrorBoundary, {
		key: storyContext.id,
		showMain,
		showException
...
var decorators = [(story, context) => {
	...
	try {
		let { configure } = await import("./test-DIuBqnUp.js").then(__toDynamicImportESM(1)), act = await getAct();
		configure({
...
//# sourceMappingURL=chunk-MNKU3DTE-Dw9ZbKky.js.map