...
import { Tn, pt } from "./docs-tools-sF2y6cHU.js";
import { applyDecorators } from "./chunk-XLZBPYSH-Q6C3gNO_.js";
import { __commonJS, __export, __toESM } from "./chunk-XP5HYGXS-wyfCjB2n.js";

//#region node_modules/@storybook/react/dist/chunk-6BNVLEVL.mjs
...
`.concat(spacer(lvl + 1, options.tabStop), "$1"));
	...
	return typeof propValue == "function" ? "{".concat(formatFunction(propValue, options), "}") : (0, import_react.isValidElement)(propValue) ? "{".concat(formatTreeNode(_parseReactElement(propValue, options), !0, lvl, options), "}") : propValue instanceof Date ? isNaN(propValue.valueOf()) ? "{new Date(NaN)}" : "{new Date(\"".concat(propValue.toISOString(), "\")}") : isPlainObject(propValue) || Array.isArray(propValue) ? "{".concat(formatComplexDataStructure(propValue, inline, lvl, options), "}") : "{".concat(String(propValue), "}");
...
//#endregion
//#region node_modules/@storybook/react/dist/chunk-6PSAWJ36.mjs
...
var renderJsx = (code, options) => {
	...
	} : displayNameDefaults = { displayName: (el) => el.type.displayName ? el.type.displayName : pt(el.type, "displayName") ? pt(el.type, "displayName") : el.type.render?.displayName ? el.type.render.displayName : typeof el.type == "symbol" || el.type.$$typeof && typeof el.type.$$typeof == "symbol" ? getReactSymbolName(el.type) : el.type.name && el.type.name !== "_default" ? el.type.name : typeof el.type == "function" ? "No Display Name" : isForwardRef(el.type) ? el.type.render.name : isMemo(el.type) ? el.type.type.name : el.type };
	let opts = {
		...displayNameDefaults,
...
`).replace(/function\s+noRefCheck\(\)\s*\{\}/g, "() => {}");
	...
	return sourceParams?.type === Tn.DYNAMIC ? !1 : !isArgsStory || sourceParams?.code || sourceParams?.type === Tn.CODE;
	...
	let jsx = (0, import_preview_api.useRef)(void 0), story = storyFn(), skip = skipJsxRender(context), options = {
		...defaultOpts,
...
var decorators = [jsxDecorator], parameters = { docs: { story: { inline: !0 } } };

//#endregion
export { applyDecorators2, decorators, entry_preview_docs_exports, isMemo, parameters, reactElementToJsxString };
//# sourceMappingURL=chunk-6PSAWJ36-l6L2IbGJ.js.map