//#region node_modules/@vitest/mocker/dist/chunk-registry.js
...
var ManualMockedModule = class ManualMockedModule {
	...
	async resolve() {
		...
		try {
			...
			const vitestError = /* @__PURE__ */ new Error("[vitest] There was an error when mocking a module. If you are using \"vi.mock\" factory, make sure there are no top level variables inside, since this call is hoisted to top of the file. Read more: https://vitest.dev/api/vi.html#vi-mock");
...
//#endregion
//#region node_modules/@vitest/mocker/dist/index.js
function mockObject(options, object, mockExports = {}) {
	...
	const mockPropertiesOf = (container, newContainer) => {
		...
		for (const { key: property, descriptor } of getAllMockableProperties(container, isModule, options.globalConstructors)) {
			...
			if (!define(newContainer, property, isFunction ? value : {})) continue;
			if (isFunction) {
				...
				function mockFunction() {
					if (this instanceof newContainer[property]) for (const { key, descriptor: descriptor$1 } of getAllMockableProperties(this, false, options.globalConstructors)) {
						if (descriptor$1.get) continue;
						const value$1 = this[key];
						if (getType(value$1).includes("Function") && typeof value$1 === "function") {
							const original = this[key];
							const mock$1 = spyOn(this, key).mockImplementation(original);
							const origMockReset = mock$1.mockReset;
							mock$1.mockRestore = mock$1.mockReset = () => {
								origMockReset.call(mock$1);
								mock$1.mockImplementation(original);
								return mock$1;
							};
						}
					}
...
function getAllMockableProperties(obj, isModule, constructors) {
	const { Map: Map$1, Object: Object$1, Function, RegExp, Array: Array$1 } = constructors;
	const allProps = new Map$1();
	let curr = obj;
	do {
		if (curr === Object$1.prototype || curr === Function.prototype || curr === RegExp.prototype) break;
		collectOwnProperties(curr, (key) => {
			const descriptor = Object$1.getOwnPropertyDescriptor(curr, key);
			if (descriptor) allProps.set(key, {
				key,
				descriptor
			});
		});
	} while (curr = Object$1.getPrototypeOf(curr));
	if (isModule && !allProps.has("default") && "default" in obj) {
		const descriptor = Object$1.getOwnPropertyDescriptor(obj, "default");
		if (descriptor) allProps.set("default", {
			key: "default",
			descriptor
		});
	}
	return Array$1.from(allProps.values());
}
...
//#endregion
export { AutomockedModule, AutospiedModule, ManualMockedModule, MockerRegistry, RedirectedModule, mockObject };
//# sourceMappingURL=dist-DK6BwQZU.js.map