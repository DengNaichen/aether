...
//#region node_modules/storybook/dist/docs-tools/index.js
var import_preview_errors = /* @__PURE__ */ __toESM(require_preview_errors());
var import_preview_errors$1 = /* @__PURE__ */ __toESM(require_preview_errors());
...
var Pr = (n, s, a) => (a = n != null ? dr(xr(n)) : {}, wr(s || !n || !n.__esModule ? Ie(a, "default", {
	value: n,
	enumerable: !0
}) : a, n));
var dt = Jr((fe, yt) => {
	(function(n, s) {
		...
		r(s, "tokenToString");
		let ne = class ne$1 extends Error {
			constructor(t) {
				super(`No parslet found for token: ${s(t)}`), this.token = t, Object.setPrototypeOf(this, ne$1.prototype);
			}
			getToken() {
				return this.token;
			}
		...
		let a = ne, oe = class oe$1 extends Error {
			constructor(t) {
				super(`The parsing ended early. The next token was: ${s(t)}`), this.token = t, Object.setPrototypeOf(this, oe$1.prototype);
			}
			getToken() {
				return this.token;
			}
		...
		let p = oe, se = class se$1 extends Error {
			constructor(t, o) {
				let i = `Unexpected type: '${t.type}'.`;
				o !== void 0 && (i += ` Message: ${o}`), super(i), Object.setPrototypeOf(this, se$1.prototype);
			}
		};
		...
		r(m, "getQuoted");
		let T = new RegExp("[$_\\p{ID_Start}]|\\\\u\\p{Hex_Digit}{4}|\\\\u\\{0*(?:\\p{Hex_Digit}{1,5}|10\\p{Hex_Digit}{4})\\}", "u"), g = new RegExp("[$\\-\\p{ID_Continue}\\u200C\\u200D]|\\\\u\\p{Hex_Digit}{4}|\\\\u\\{0*(?:\\p{Hex_Digit}{1,5}|10\\p{Hex_Digit}{4})\\}", "u");
		function P(e) {
			let t = e[0];
		...
		let z = /* @__PURE__ */ r((e) => {
			...
			static create(t) {
				...
				t = o.text;
				let i = this.read(t);
				return t = i.text, new U$1(t, void 0, o.token, i.token);
			}
			constructor(t, o, i, l) {
			...
			advance() {
				let t = U$1.read(this.text);
				return new U$1(t.text, this.current, this.next, t.token);
			}
		};
		r(U, "Lexer");
		let xe = U;
		function J(e) {
			...
			if (e.type === "JsdocTypeKeyValue" || e.type === "JsdocTypeParameterList" || e.type === "JsdocTypeProperty" || e.type === "JsdocTypeReadonlyProperty" || e.type === "JsdocTypeObjectField" || e.type === "JsdocTypeJsdocObjectField" || e.type === "JsdocTypeIndexSignature" || e.type === "JsdocTypeMappedType") throw new c(e);
			return e;
		}
				...
				switch (k.type) {
					...
					case "JsdocTypeStringValue":
						G = {
							type: "JsdocTypeProperty",
							value: k.value,
							meta: { quote: k.meta.quote }
						};
						break;
					case "JsdocTypeSpecialNamePath":
						if (k.specialType === "event") G = k;
						else throw new c(k, "Type 'JsdocTypeSpecialNamePath' is only allowed with specialType 'event'");
						break;
					default: throw new c(k, "Expecting 'JsdocTypeName', 'JsdocTypeNumber', 'JsdocStringValue' or 'JsdocTypeSpecialNamePath'");
				}
				if (ae && !l.consume("]")) {
				...
				parsePrefix: /* @__PURE__ */ r((o) => {
					...
					if (!o.consume("}")) {
						...
						for (;;) {
							...
							} else if (d.type === "JsdocTypeObjectField" || d.type === "JsdocTypeJsdocObjectField") i.elements.push(d);
							else throw new c(d);
							if (o.lexer.current.startOfLine) l = "linebreak";
							else if (o.consume(",")) l = "comma";
							else if (o.consume(";")) l = "semicolon";
							else break;
		...
		function Qe(e, t) {
			switch (t) {
				case "closure": return new I(Yt, e).parse();
				case "jsdoc": return new I($t, e).parse();
				case "typescript": return new I(nr, e).parse();
			}
		}
		r(Qe, "parse");
		function or(e, t = [
		...
		function j(e, t) {
			switch (t) {
				case "double": return `"${e}"`;
				case "single": return `'${e}'`;
				case void 0: return e;
			}
		}
		r(j, "quote");
		function et() {
			return {
				...
				JsdocTypeNamePath: /* @__PURE__ */ r((e, t) => {
					...
					switch (e.pathType) {
						case "inner": return `${o}~${i}`;
						case "instance": return `${o}#${i}`;
						case "property": return `${o}.${i}`;
						case "property-brackets": return `${o}[${i}]`;
					}
				}, "JsdocTypeNamePath"),
		...
		let pr = {
			...
			JsdocTypeAny: /* @__PURE__ */ r(() => ({ type: "AllLiteral" }), "JsdocTypeAny"),
			JsdocTypeNull: /* @__PURE__ */ r(() => ({ type: "NullLiteral" }), "JsdocTypeNull"),
			JsdocTypeStringValue: /* @__PURE__ */ r((e) => F(j(e.value, e.meta.quote)), "JsdocTypeStringValue"),
			JsdocTypeUndefined: /* @__PURE__ */ r(() => ({ type: "UndefinedLiteral" }), "JsdocTypeUndefined"),
			JsdocTypeUnknown: /* @__PURE__ */ r(() => ({ type: "UnknownLiteral" }), "JsdocTypeUnknown"),
			JsdocTypeFunction: /* @__PURE__ */ r((e, t) => {
				let o = Ze(e), i = {
					type: "FunctionType",
			...
			JsdocTypeSymbol: /* @__PURE__ */ r((e) => {
				...
				return o?.type === "JsdocTypeVariadic" && (o.meta.position === "prefix" ? t = "..." : i = !0, o = o.element), o?.type === "JsdocTypeName" ? t += o.value : o?.type === "JsdocTypeNumber" && (t += o.value.toString()), i && (t += "..."), F(`${e.value}(${t})`);
			}, "JsdocTypeSymbol"),
		...
		let ur = {
			...
			JsdocTypeAny: /* @__PURE__ */ r(() => ({ type: "ANY" }), "JsdocTypeAny"),
			JsdocTypeFunction: /* @__PURE__ */ r((e, t) => {
				let o = Ze(e), i = {
			...
			JsdocTypeUnknown: /* @__PURE__ */ r(() => ({ type: "UNKNOWN" }), "JsdocTypeUnknown"),
		...
		function ke(e, t, o, i, l) {
			...
			for (let d of f) {
				let h = e[d];
				if (h !== void 0) if (Array.isArray(h)) for (let D of h) ke(D, e, d, i, l);
				else ke(h, e, d, i, l);
			}
			l?.(e, t, o);
		}
...
var Sr = /* @__PURE__ */ r((n) => n.name === "literal", "isLiteral"), Er = /* @__PURE__ */ r((n) => n.value.replace(/['|"]/g, ""), "toEnumOption"), Nr = /* @__PURE__ */ r((n) => {
	...
	switch (typeof a < "u" && (p.raw = a), n.name) {
		...
		case "union": return n.elements?.every(Sr) ? {
			...p,
			name: "enum",
...
var Ar = /* @__PURE__ */ r((n) => {
	switch (n.type) {
		...
		default: throw new import_preview_errors$1.UnknownArgTypesError({
			type: n,
			language: "Typescript"
		});
	}
...
function Be({ startLine: n = 0, fence: s = "```", spacing: a = "compact", markers: p = v, tokenizers: c = [
	...
	let u = Ve({
		startLine: n,
		markers: p
	}), m = _e({ fence: s }), T = Le({ tokenizers: c }), g = Ue(a);
	return function(P) {
		let b = [];
		for (let de of ut(P)) {
			let q = u(de);
...
function Gr(n, s) {
	...
	for (let p of n.tags) if (!(s !== void 0 && !s.includes(p.tag))) if (p.tag === "ignore") {
		...
		case "argument": {
			let c = zr(p);
			c != null && (a.params ??= [], a.params.push(c));
			break;
		}
		case "deprecated": {
			let c = Hr(p);
...
function St(n) {
	...
	switch (n.name) {
		case "union": return en(n);
		case "signature": return nn(n);
		default: return on(n);
	}
}
r(St, "createType");
...
function pn(n) {
	return n ? n.name === "string" ? !0 : n.name === "enum" ? Array.isArray(n.value) && n.value.every(({ value: s }) => typeof s == "string" && s[0] === "\"" && s[s.length - 1] === "\"") : !1 : !1;
}
r(pn, "isStringValued");
function cn(n, s) {
	if (n != null) {
...
var ln = /* @__PURE__ */ r((n, s, a) => {
	...
	switch (n) {
		case "JavaScript": return ln;
		case "TypeScript": return un;
		case "Flow": return mn;
		default: return fn;
	}
}, "getPropDefFactory");
var kt = /* @__PURE__ */ r((n) => n.type != null ? "JavaScript" : n.flowType != null ? "Flow" : n.tsType != null ? "TypeScript" : "Unknown", "getTypeSystem"), yn = /* @__PURE__ */ r((n) => {
	let s = kt(n[0]), a = $e(s);
	return n.map((p) => {
		let c = p;
...
var ya = /* @__PURE__ */ r((n) => {
	let { component: s, argTypes: a, parameters: { docs: p = {} } } = n, { extractArgTypes: c } = p;
	if (!c || !s) return a;
	let u = c(s);
	return u ? qe(u, a) : a;
}, "enhanceArgTypes");
var It = "storybook/docs", ga = `${It}/panel`, xa = "docs", ha = `${It}/snippet-rendered`, Tn = /* @__PURE__ */ ((p) => (p.AUTO = "auto", p.CODE = "code", p.DYNAMIC = "dynamic", p))(Tn || {});

//#endregion
export { Ir, Ke, Tn, aa, at, ha, ia, pt, w, wt, ya };
//# sourceMappingURL=docs-tools-sF2y6cHU.js.map