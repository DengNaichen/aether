...
import { AutomockedModule, MockerRegistry, RedirectedModule, mockObject } from "./dist-DK6BwQZU.js";

//#region node_modules/@vitest/mocker/dist/chunk-pathe.M-eThtNZ.js
...
var isAbsolute$1 = function(p$1) {
	return _IS_ABSOLUTE_RE$1.test(p$1);
};
var extname = function(p$1) {
	if (p$1 === "..") return "";
	const match = _EXTNAME_RE.exec(normalizeWindowsPath$1(p$1));
	return match && match[1] || "";
};

//#endregion
//#region node_modules/@vitest/mocker/dist/chunk-mocker.js
var f = {
	reset: [0, 0],
	bold: [
		1,
...
function parseSingleV8Stack(raw) {
	...
	const location$1 = sanitizedLine.match(/ (\(.+\)$)/);
	sanitizedLine = location$1 ? sanitizedLine.replace(location$1[0], "") : sanitizedLine;
	const [url, lineNumber, columnNumber] = extractLocation(location$1 ? location$1[1] : sanitizedLine);
	let method = location$1 && sanitizedLine || "";
...
function getImporter(name) {
	const stackArray = (/* @__PURE__ */ createSimpleStackTrace({ stackTraceLimit: 5 })).split("\n");
...
function rpc(event, data) {
	...
	return new Promise((resolve$1, reject) => {
		const timeout = setTimeout(() => {
			reject(/* @__PURE__ */ new Error(`Failed to resolve ${event} in time`));
		}, 5e3);
		hot.on(`${event}:result`, function r(data$1) {
			resolve$1(data$1);
...
var ModuleMocker = class {
	...
	constructor(interceptor, rpc$1, spyOn, config) {
	...
	wrapDynamicImport(moduleFactory) {
		if (typeof moduleFactory === "function") return new Promise((resolve$1, reject) => {
			this.prepare().finally(() => {
				moduleFactory().then(resolve$1, reject);
			});
		});
...
//#endregion
//#region node_modules/@vitest/mocker/dist/chunk-utils.js
var postfixRE = /[?#].*$/;
...
function createManualModuleSource(moduleUrl, exports, globalAccessor = "\"__vitest_mocker__\"") {
	...
	const keys = exports.map((name) => {
		if (name === "default") return `export default module["default"];`;
...
//#endregion
//#region node_modules/@vitest/mocker/dist/chunk-interceptor-native.js
...
//#endregion
//#region node_modules/@vitest/mocker/dist/browser.js
var ModuleMockerMSWInterceptor = class {
	...
	constructor(options = {}) {
		...
		if (!options.globalThisAccessor) options.globalThisAccessor = "\"__vitest_mocker__\"";
	}
	async register(module) {
		await this.init();
	...
	async init() {
		...
		this.startPromise = Promise.all([worker ? { setupWorker(handler) {
			worker.use(handler);
			return worker;
		} } : import("./mocker-DWK0zlT_.js").then(__toDynamicImportESM(1)), import("./mocker-CGPvws0L.js").then(__toDynamicImportESM(1))]).then(([{ setupWorker }, { http }]) => {
			const worker$1 = setupWorker(http.get(/.+/, async ({ request }) => {
				const path = cleanQuery(request.url.slice(location.origin.length));
				if (!this.mocks.has(path)) return passthrough();
			...
			return worker$1.start(this.options.mswOptions).then(() => worker$1);
		}).finally(() => {
			this.worker = worker;
...
function injectQuery(url, queryToInject) {
	const { search, hash } = new URL(url.replace(replacePercentageRE, "%25"), location.href);
	return `${cleanUrl(url)}?${queryToInject}${search ? `&${search.slice(1)}` : ""}${hash ?? ""}`;
}

//#endregion
export { ModuleMocker, ModuleMockerMSWInterceptor, ModuleMockerServerInterceptor, createCompilerHints };
